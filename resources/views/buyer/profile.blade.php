    ...
    <div class="container mx-auto px-4 md:px-8 pb-12">
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
                ...
                <div class="bg-white border border-gray-100 rounded-xl p-6 shadow-sm">
                    ...
                    <div class="flex flex-col items-center mb-6 pb-6 border-b border-gray-100">
                        <div class="relative group">
                            <div class="w-20 h-20 md:w-24 md:h-24 rounded-full shadow-lg overflow-hidden ring-4 ring-white">
                                @if($user->file('foto_profil')->hasFile())
                                    <img src="{{ $user->file('foto_profil')->preview() }}" alt="{{ $user->name }}"
                                        class="w-full h-full object-cover">
                                @else
                                    <div
                                        class="w-full h-full bg-gradient-to-br from-amber-600 to-orange-600 flex items-center justify-center">
                        ...
                        <h3 class="text-lg md:text-xl font-bold text-gray-900 mt-3">{{ $user->name ?? 'Nama Belum Diisi' }}
                        </h3>
                        ...
                        <form id="avatarForm" action="{{ route('profile.avatar') }}" method="POST"
                            ...
                            @csrf
                            <input type="file" id="avatarInput" name="upload_foto_profil" accept="image/*"
                                onchange="previewAndUpload(event)">
                        </form>
                    </div>
                    ...
                    <nav class="space-y-2">
                        <a href="#"
                            class="flex items-center gap-3 px-4 py-3 bg-amber-50 text-amber-700 rounded-lg font-semibold group transition">
                            <i class="fa-solid fa-user text-lg"></i>
                            <span>Informasi Profil</span>
                        </a>
                        <a href="#password-section" onclick="showPasswordSection()"
                            class="flex items-center gap-3 px-4 py-3 text-gray-600 hover:bg-gray-50 rounded-lg font-medium group transition">
                        ...
                        <form action="{{ route('logout') }}" method="POST" class="w-full">
                            @csrf
                            <button type="submit"
                                class="w-full flex items-center gap-3 px-4 py-3 text-red-600 hover:bg-red-50 rounded-lg font-medium group transition">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div id="profile-section" class="bg-white border border-gray-100 rounded-xl p-6 md:p-8 shadow-sm">
                    <div class="flex items-center justify-between mb-6">
                        ...
                        <button onclick="toggleEditMode()" id="edit-btn"
                            class="btn btn-sm bg-amber-600 hover:bg-amber-700 text-white border-none rounded-full">
                    ...
                    {{-- EDIT MODE --}}
                    <form id="edit-mode" action="{{ route('profile.update') }}" method="POST" class="space-y-5"
                        style="display: none;">
                        @csrf
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-2">
                            ...
                            <div class="md:col-span-2">
                                <input type="text" name="no_hp" value="{{ $user->no_hp }}"
                                    class="input input-bordered w-full rounded-lg" placeholder="Contoh: 081234567890">
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-2">
                            <label class="text-sm font-semibold text-gray-700">Alamat</label>
                            <div class="md:col-span-2">
                                <textarea name="alamat" rows="3" class="textarea textarea-bordered w-full rounded-lg"
                        ...
                        <div class="flex justify-end gap-2 mt-6">
                            <button type="button" onclick="toggleEditMode()"
                                class="btn btn-sm bg-gray-500 hover:bg-gray-600 text-white border-none rounded-full">
                                <i class="fa-solid fa-times"></i> Batal
                            </button>
                            <button type="submit"
                                class="btn btn-sm bg-green-600 hover:bg-green-700 text-white border-none rounded-full">
                ...
                {{-- UBAH PASSWORD --}}
                <div id="password-section" class="bg-white border border-gray-100 rounded-xl p-6 md:p-8 shadow-sm"
                    style="display: none;">
                    <div class="flex items-center justify-between mb-6">
                        <h2 class="text-xl md:text-2xl font-bold text-gray-900">Ubah Password</h2>
                        <button onclick="hidePasswordSection()"
                            class="btn btn-sm bg-gray-500 hover:bg-gray-600 text-white border-none rounded-full">
                    ...
                    <form action="{{ route('profile.password') }}" method="POST" class="space-y-5">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-2">
                            ...
                            <div class="md:col-span-2">
                                <input type="password" name="new_password" class="input input-bordered w-full rounded-lg"
        ...
        function previewAndUpload(event) {
            ...
            if (file) {
                if (!file.type.startsWith('image/')) {
                    Swal.fire({ icon: 'error', title: 'File Tidak Valid', text: 'Silakan pilih file gambar' });
                    return;
                }
                if (file.size > 2 * 1024 * 1024) {
                    Swal.fire({ icon: 'error', title: 'File Terlalu Besar', text: 'Ukuran maksimal 2 MB' });
                    return;
                }
