...
@section('content')
    ...
    {{-- SECTION 2: KATEGORI PILIHAN (Static UI) --}}
    <div class="container mx-auto px-4 md:px-8 py-8">
        <div class="flex justify-between items-end mb-6">
            <h2 class="font-bold text-xl md:text-2xl text-gray-800">Kategori Pilihan</h2>
            <a href="#" class="text-amber-600 text-sm font-semibold hidden md:block hover:underline">Lihat Semua Kategori</a>
        </div>
        <div class="flex md:grid md:grid-cols-4 gap-3 overflow-x-auto hide-scroll pb-2">
            <button class="flex-shrink-0 btn bg-amber-600 hover:bg-amber-700 text-white border-none rounded-xl font-medium shadow-md">
                <i class="fa-solid fa-fire mr-2"></i> Best Seller
            </button>
            <button class="flex-shrink-0 btn bg-white hover:bg-gray-50 text-gray-600 border-gray-200 rounded-xl font-medium">
                🍪 Kue Basah
            </button>
            <button class="flex-shrink-0 btn bg-white hover:bg-gray-50 text-gray-600 border-gray-200 rounded-xl font-medium">
                🍘 Keripik
    ...
    {{-- SECTION 3: REKOMENDASI WISATAWAN (Dynamic Loop) --}}
    <div class="container mx-auto px-4 md:px-8 pb-12">
        <h2 class="font-bold text-xl md:text-2xl mb-6 text-gray-800">Rekomendasi Wisatawan 🔥</h2>
            ...
            @forelse($data_barang as $item)
                <div class="group bg-white border border-gray-100 rounded-xl p-3 md:p-4 shadow-sm hover:shadow-xl transition duration-300">
                    ...
                    <div class="relative aspect-square bg-gray-100 rounded-lg mb-3 overflow-hidden">
                        {{-- Logika Gambar: Gunakan asset storage jika ada, jika tidak pakai placeholder --}}
                        <img src="{{ $item->gambar ? asset('storage/' . $item->gambar) : 'https://via.placeholder.com/300?text=No+Image' }}" 
                             alt="{{ $item->nama_barang }}" 
                             class="w-full h-full object-cover group-hover:scale-110 transition duration-500">
                        
                        {{-- Badge Promo (Hardcoded Logic Visual: Anggap Id Ganjil = Promo) --}}
                        @if($item->id % 2 != 0) 
                            <span class="absolute top-2 right-2 bg-red-500 text-white text-[10px] font-bold px-2 py-1 rounded shadow">PROMO</span>
                        @endif
                    </div>
                    ...
                    <div class="flex justify-between items-end mt-2">
                        <div>
                            ...
                            @if($item->id % 2 != 0)
                                <p class="text-xs text-gray-400 line-through">Rp {{ number_format($item->harga * 1.2, 0, ',', '.') }}</p>
                            @endif
                        ...
                        {{-- Button Add --}}
                        <button class="bg-gray-900 text-white w-8 h-8 md:w-10 md:h-10 rounded-full flex items-center justify-center hover:bg-amber-600 transition shadow-lg transform active:scale-90">
